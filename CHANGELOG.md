...
`LAST_CREDENTIAL`, kind ``LAST_CREDENTIAL``. An account with no credentials still
...
`identities.size + (if (hasPassword) 1 else 0) <= 1`. The error is the backstop, not the UX.
